...
module.exports = async (req, res) => {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // GET: fetch trade reports + analytics  —  /api/manage-trades
  // ══════════════════════════════════════════════════════════════════════════
  if (req.method === 'GET') {
    try {
      ...
      const tp1Count    = reports.filter(r => r.events?.some(e => e.type === 'PARTIAL_CLOSE_TP1')).length;
      const tp2Count    = reports.filter(r => r.events?.some(e => e.type === 'PARTIAL_CLOSE_TP2')).length;
      const tp3Count    = reports.filter(r => r.events?.some(e => e.type === 'FULL_CLOSE_TP3')).length;
      ...
      for (const r of reports) {
        ...
        if (r.events?.some(e => e.type === 'PARTIAL_CLOSE_TP1')) byInstrument[sym].tp1++;
        if (r.events?.some(e => e.type === 'PARTIAL_CLOSE_TP2')) byInstrument[sym].tp2++;
        if (r.events?.some(e => e.type === 'FULL_CLOSE_TP3'))    byInstrument[sym].tp3++;
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // POST: manage open positions (original logic)
  // ══════════════════════════════════════════════════════════════════════════
...
const pnl = parseFloat((profit_distance * closeVolume * pipValue).toFixed(2));
        ...
        if (closeRes.ok && (closeData.orderId || closeData.positionId)) {
          ...
          if (breakeven && stopLoss !== breakeven) {
            ...
            if (modRes.ok) { result.actions.push({ type: 'SL_TO_BREAKEVEN', level: breakeven }); state.beSet = true; }
          }
        }
      }
...
const pnl = parseFloat((profit_distance * closeVolume * pipValue).toFixed(2));
        ...
        if (closeRes.ok && (closeData.orderId || closeData.positionId)) {
          ...
          if (pos.atr) {
            const trailSL = direction === 'LONG' ? currentPrice - pos.atr : currentPrice + pos.atr;
            const modRes  = await fetch(`${BASE}/trade`, {
              method: 'POST', headers,
              body: JSON.stringify({ actionType: 'POSITION_MODIFY', positionId: id, stopLoss: parseFloat(trailSL.toFixed(5)), comment: 'QuantumBot:TRAIL_TP2' })
            });
