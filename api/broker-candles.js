/* eslint-disable */
// api/broker-candles.js -- V9 dynamic symbol resolution
...
module.exports = async (req, res) => {
  ...
  for (const trySym of candidates) {
    try {
      const url =
        'https://mt-market-data-client-api-v1.london.agiliumtrade.ai/users/current/accounts/' + ACCOUNT_ID +
      ...
      if (redis && candles.length > 0) { try { await redis.set('v9:sym:' + baseSym, JSON.stringify(trySym)); } catch (_) {} }

      return res.status(200).json({ candles, count: candles.length, symbol: trySym, timeframe, source: 'puprime' });
    } catch (e) { lastErr = e.message; }
  }

  return res.status(404).json({ error: lastErr || 'Symbol not found on broker', candles: [], tried: candidates });
};